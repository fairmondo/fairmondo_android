...
public class SearchableActivity extends AbstractActivity implements OnSearchResultListener {
    ...
    public void init() {
        ...
        // TODO why does ACTION_VIEW not match? The strings are equal
        switch (action) {
    ...
    private void showResult(Uri data) {
        if (app.isConnected()) {
            ...
            restCommunicator.getProduct(searchSuggestion.getSuggest_text_1(), category.getId().intValue());
        } else {
    ...
    private void searchProducts(String query) {
        if (app.isConnected()) {
            if (query != null) {
                restCommunicator.getProduct(query);
            } else {
                this.finish();
            }
        } else {
